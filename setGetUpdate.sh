...
#Created from modified script by Austin and formatting help from Marlena.
...
searchBlockchain() # Cant curl the etherscan link for the api but, pulls up in browser fine.
{
    ...
    read STARTBLOCK
    echo "Enter in Etherscan api key"
    read  ETHERSCANAPIKEY
    #STARTBLOCK=$(echo $STARTBLOCK | xxd -pu )
    echo $STARTBLOCK >&2
    TRANSACTIONHASHES=$(curl http://rinkeby.etherscan.io/api?module=account&action=txlist&address="0x8e289aD72dE59555dC2c8117e11De721819F1B2a"&startblock="726450"&endblock="99999999"&sort=asc&apikey="J26AM7SXS7QT4SNWX5HKUD36SNQVCH9S1J"'{"jsonrpc":"2.0"}') | jq -r '.status' 
    #FILTER=$(curl http://127.0.0.1:8545 -X POST --data '{"method":"eth_newFilter","params":[{"fromBlock":"0x'$STARTBLOCK'","toBlock":"latest","address":"'$CONTRACTADDRESS'"}],"id":1,"jsonrpc":"2.0"}'| jq -r '.result') 
    #echo $FILTER >&2
    #TRANSACTIONHASHES=$(curl 127.0.0.1:8545 -X POST --data '{"method":"eth_getFilterChanges","params":["'$FILTER'"],"jsonrpc":"2.0","id":1}') #| jq -r '.result') 
    echo $TRANSACTIONHASHES >&2
}
updatePackages()
{
    echo "Need to think about what to do. Will make later."
    echo "Still need more time to think"
}
...
if [ $CHOICE == "1" ]
then
     echo "Enter in echo.sol contract." # This is where you enter echo.sol or path to echo.sol. 
